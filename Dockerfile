...
# Clone repository
RUN git clone https://github.com/vacanza/holidays /workspace/repo
...
RUN echo '#!/bin/bash' > /tmp/setup.sh && \
    ...
    echo "ls -l requirements" >> /tmp/setup.sh && \
    echo "cat requirements.txt || true" >> /tmp/setup.sh && \
    echo "cat requirements/README.md || true" >> /tmp/setup.sh && \
    echo "cat tox.ini" >> /tmp/setup.sh && \
    echo "cat pyproject.toml" >> /tmp/setup.sh && \
    echo "cat requirements/build.txt" >> /tmp/setup.sh && \
    echo "cat requirements/runtime.txt" >> /tmp/setup.sh && \
    echo "cat requirements/dev.txt" >> /tmp/setup.sh && \
    echo "cat requirements/docs.txt" >> /tmp/setup.sh && \
    echo "cat requirements/tests.txt" >> /tmp/setup.sh && \
    echo "pip install -e ." >> /tmp/setup.sh && \
    echo "pip install -r requirements/build.txt -r requirements/dev.txt -r requirements/docs.txt -r requirements/runtime.txt -r requirements/tests.txt" >> /tmp/setup.sh && \
    echo "python -c \"import holidays; print(holidays.__version__)\"" >> /tmp/setup.sh && \
